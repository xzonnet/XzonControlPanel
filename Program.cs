    ...
    class Program
    {
        ...
        static void Main()
        {
            ...
            try
            {
                ...
                //var process = processes.FirstOrDefault(p => p.ProcessName == "OpenHardwareMonitor");
                //if (process == null)
                //{
                //    PrettyConsole.WriteLine("OpenHardwareMonitor.exe is not running. Starting it now.", ConsoleColor.Yellow);
                //
                //    ProcessStartInfo pInfo =
                //        new ProcessStartInfo("External\\OpenHardwareMonitor\\OpenHardwareMonitor.exe")
                //        {
                ...
                while (_running)
                {
                    ...
                    {
                        ProcessHelper.KillProcess("OpenHardwareMonitor");
                        continue;
                    }
        ...
        private static void UpdateMonitoringSockets(List<Gpu> gpus, List<Cpu> cpus, decimal averageHashrate, ConsoleColor hashrateColor)
        {
            ...
            foreach (var channel in _channels)
            {
                var r = new MiningRig
                {
                    ...
                    Name = channel.ShowRigName ? RigName : FingerPrint.Value,
                    Cpus = cpus,
                    Gpus = gpus,
                    Stats = new List<Stats>
                    {
                        new Stats
                        {
                            Code =  _currentMiningConfig.CurrencyCode ?? string.Empty,
                ...
                //TODO: DEBUG
                //File.WriteAllText($"{channel.Name}_pulse.json", json);
        ...
        private static void VerifyProcesses()
        {
            ...
            if (_minerProcess == null && !_paused)
            {
                ...
                if (_commandLineProcess != null)
                {
                    ...
                    //TODO: See if autoflush is needed
                    //_commandLineProcess.StandardInput.AutoFlush = true;
                }
            ...
            if (process == null || process.HasExited)
            {
                ...
                //Relaunch OpenHardwareMonitor
                PrettyConsole.WriteLine("OpenHardwareMonitor.exe is not running. Starting it now.", ConsoleColor.Yellow);

                //if (_minerProcess != null)
                //{
                //    PrettyConsole.WriteLine("Freezing miner process.", ConsoleColor.Magenta);
                //    ProcessHelper.SuspendProcess(_minerProcess.Id);
                //}
                ...
                //if (_minerProcess != null)
                //{
                //    PrettyConsole.WriteLine("Unfreezing miner process.", ConsoleColor.Magenta);
                //    ProcessHelper.ResumeProcess(_minerProcess.Id);
                //}
        ...
        private static void ConnectToMonitoringSocket()
        {
            ...
            _monitoringSocket.On(Socket.EVENT_CONNECT, () =>
            {
                ...
                foreach (var channel in _channels)
                {
                    ...
                    //TODO: DEBUG
                    //File.WriteAllText($"{channel.Name}_auth.json", json);
